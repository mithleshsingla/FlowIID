...
def get_time_discretization(nfes: int, rho=7):
    if nfes == 1:
        # Just return [0, 1] when only 1 step requested
    ...
    t_samples = torch.cat([torch.tensor([0.0], dtype=t_samples.dtype), t_samples])
    return t_samples
...
def calculate_psnr(pred, target, mask=None, max_value=1.0):
    """Calculate PSNR with optional masking"""
    ...
    if mask is not None:
        # FIXED: Handle dimension mismatch more robustly
        if pred.ndim == 3 and mask.ndim == 3:  # Both are (C,H,W)
            if mask.shape[0] == 1 and pred.shape[0] > 1:
                mask = np.broadcast_to(mask, pred.shape)
            elif mask.shape[0] > 1 and pred.shape[0] == 1:
                mask = mask[:1]  # Take first channel
        elif pred.ndim == 3 and mask.ndim == 2:  # pred is (C,H,W), mask is (H,W)
            mask = np.broadcast_to(mask[None, :, :], pred.shape)
        elif pred.ndim == 4 and mask.ndim == 4:  # Both are (B,C,H,W)
            if mask.shape[1] == 1 and pred.shape[1] > 1:
                mask = np.broadcast_to(mask, pred.shape)
        elif pred.ndim == 4 and mask.ndim == 3:  # pred is (B,C,H,W), mask is (B,H,W) or (C,H,W)
            if mask.shape[0] == pred.shape[0]:  # (B,H,W)
                mask = mask[:, None, :, :].repeat(pred.shape[1], axis=1)
            else:  # (C,H,W) -> assume single batch
                mask = mask[None, :, :, :].repeat(pred.shape[0], axis=0)
        elif pred.ndim == 4 and mask.ndim == 2:  # pred is (B,C,H,W), mask is (H,W)
            mask = np.broadcast_to(mask[None, None, :, :], pred.shape)
...
def calculate_psnr_(pred, target, mask=None, max_value=1.0):
    """Calculate PSNR with optional masking"""
    # Convert tensors to numpy if needed
    if torch.is_tensor(pred):
        pred = pred.cpu().numpy()
    ...
    if mask is not None:
        # Handle dimension mismatch - expand mask to match pred/target dimensions
        if pred.ndim == 3 and mask.ndim == 2:  # pred is (C,H,W), mask is (H,W)
            mask = np.broadcast_to(mask[None, :, :], pred.shape)
        elif pred.ndim == 4 and mask.ndim == 3:  # pred is (B,C,H,W), mask is (B,H,W)
            mask = np.broadcast_to(mask[:, None, :, :], pred.shape)
        elif pred.ndim == 4 and mask.ndim == 2:  # pred is (B,C,H,W), mask is (H,W)
            mask = np.broadcast_to(mask[None, None, :, :], pred.shape)
...
def calculate_scale_invariant_mse_author2_style(pred, target, valid_pixels):
    ...
    try:
        ...
        #print(f"Pred shape: {pred_np.shape}, Target shape: {target_np.shape}, Mask shape: {mask_np.shape}")
        # Extract valid pixels
        pred_masked = pred_np[mask_np]
        target_masked = target_np[mask_np]
        #print(f"Masked Pred shape: {pred_masked.shape}, Masked Target shape: {target_masked.shape}")
...
def calculate_metrics_with_mask_author2_style(pred, target, mask, metric_name_prefix="", window_size=20):
    ...
    for i in range(batch_size):
        ...
        psnr_val = calculate_psnr(
            pred_batch.numpy() if torch.is_tensor(pred_batch) else pred_batch,
            target_batch.numpy() if torch.is_tensor(target_batch) else target_batch,
            mask_batch[0].numpy() if torch.is_tensor(mask_batch) else mask_batch[0],
            max_value=1.0
        )
    ...
    psnr_finite = [x for x in psnr_values if not np.isnan(x) and np.isfinite(x)]
    return {
        ...
        f'{metric_name_prefix}PSNR': float(np.mean(psnr_finite)) if psnr_finite else 0.0
    }

def calculate_ssim_simple(pred, target, mask,data_range=1.0):
    """
    Simplified SSIM calculation (no masking).
    Works with tensors shaped (1,H,W) or (3,H,W).
    """
    try:
        # Convert to numpy
        ...
        # Case 2: (3,H,W) -> transpose to (H,W,3)
            ...
            try:
                ...
                ssim_val = ssim(target, pred, data_range=data_range, channel_axis=-1)
            except TypeError:
                # Old API (scikit-image < 0.19)
                ssim_val = ssim(target, pred, data_range=data_range, multichannel=True)
...
def calculate_ssim_simple_(pred, target, mask, data_range=1.0):
    """
...
def calculate_reconstruction_loss_author2_style(predicted_albedo, predicted_shading_01, original_ldr, mask=None):
    """
    ...
    predicted_shading_01 = torch.clamp(predicted_shading_01, min=0.0, max=10.0)
    
    # Simple multiplication (Author 2's typical approach)
    reconstructed_ldr = predicted_albedo * predicted_shading_01
...
def calculate_albedo_from_hdr_shading(hdr_image, shading_image, epsilon=1e-6):
    """Calculate albedo from HDR image and shading"""
    hdr_01 = (hdr_image + 1.0) / 2.0
    hdr_01 = torch.clamp(hdr_01, min=0.0, max=1.0)
    shading_image = torch.clamp(shading_image, min=0.0, max=1.0)
    
    
    albedo=hdr_01 / (shading_image )
    return albedo
    ...
    with torch.no_grad():
        for guidance_scale in tqdm(guidance_scales, desc="Guidance Scales"):
            for method in tqdm(methods, desc="Methods", leave=False):
                ...
                all_metrics = {
                    'Albedo_SI_MSE': [], 'Albedo_SI_RMSE': [], 'Albedo_LMSE': [], 'Albedo_SSIM': [], 'Albedo_RMSE': [], 'Albedo_Valid_Pixels': [],'Albedo_PSNR': [], 
                ...
                for batch_idx, batch_data in enumerate(tqdm(triplet_loader, desc=f"Batches", leave=False)):
                    try:
                        ...
                        with torch.amp.autocast(device_type='cuda'):
                            ...
                            # T = get_time_discretization(1/guidance_scale, rho=5).to(device)
                            # print(T)
                            ...
                            predicted_shading = vae.decoder(samples.float())

                            h, mu, logvar=vae.encoder(shading_im.float())
                            latent = reparameterize(mu, logvar)
        
                            noise = torch.randn_like(latent).to(device)
                            path = AffineProbPath(scheduler=CondOTScheduler())
                            for t_val in [0,0.1]:
                                t = torch.full((latent.shape[0],), t_val, device=device)
                                path_sample = path.sample(noise, latent, t)
                                pred = model(path_sample.x_t, t, cond_encoded)
                                error = torch.nn.functional.mse_loss(pred, path_sample.dx_t)
                                print(f"t={t_val}: error={error.item()}")
                            # for t_val in [0.01, 0.05, 0.1, 0.3, 0.5, 0.7, 0.9]:
                            #     t = torch.full((latent.shape[0],), t_val, device=device, dtype=latent.dtype)
                            #     path_sample = path.sample(noise, latent, t)
                            #     x = path_sample.x_t
                            #     dx = path_sample.dx_t

                            #     pred = model(x, t, cond_encoded)

                            #     mse = torch.nn.functional.mse_loss(pred, dx, reduction='mean').item()
                            #     dx_norm = dx.view(dx.shape[0], -1).norm(dim=1).mean().item()
                            #     pred_norm = pred.view(pred.shape[0], -1).norm(dim=1).mean().item()
                            #     rel_mse = mse / (dx_norm**2 + 1e-12)

                            #     corr = ((pred.view(pred.shape[0], -1) * dx.view(dx.shape[0], -1)).sum(dim=1) /
                            #             (pred.view(pred.shape[0], -1).norm(dim=1) * dx.view(dx.shape[0], -1).norm(dim=1) + 1e-12)).mean().item()

                            #     print(f"t={t_val:.2f} | mse={mse:.4f} | dx_norm={dx_norm:.4f} | pred_norm={pred_norm:.4f} | rel_mse={rel_mse:.4f} | cos={corr:.4f}")
                        
                        predicted_shading_01 = (predicted_shading + 1.0) / 2.0
                        predicted_shading_01 = torch.clamp(predicted_shading_01, 0.0, 1.0)

                        calculated_albedo = calculate_albedo_from_hdr_shading(hdr_im, predicted_shading_01)
                        calculated_albedo = torch.clamp(calculated_albedo, 0.0, 1.0)
                        ...
                        reconstruction_metrics, reconstructed_ldr = calculate_reconstruction_loss_author2_style(
                            calculated_albedo, predicted_shading_01, ldr_im, albedo_mask
                        )
                        
                        if batch_idx == 0:
                            print(f"  Predicted albedo range: [{calculated_albedo.min():.3f}, {calculated_albedo.max():.3f}]")
                            print(f"  Calculated shading range: [{predicted_shading_01.min():.3f}, {predicted_shading_01.max():.3f}]")
                        ...
                        albedo_metrics = calculate_metrics_with_mask_author2_style(
                            calculated_albedo, albedo_im, albedo_mask, "Albedo_"
                        )
                        shading_metrics = calculate_metrics_with_mask_author2_style(
                            predicted_shading_01, shading_im, albedo_mask, "Shading_"
                        )
                            ...
                            for i in range(samples_to_save):
                                try:
                                    filepath = save_visualization_grid_with_reconstruction(
                                        ...
                                        calculated_albedo, predicted_shading_01, reconstructed_ldr,
...
def main(args):
    ...
    if os.path.exists(vae_path):
        ...
        return
    #checkpoint_path = "/home/project/ldr_image_to_ldr_shading/LDR_image_to_LDR_shading_hyperism/train_vae_mithlesh/ldr_to_sh_flow_1/epoch_71_flow_model_ckpt.pth"
    # Load flow model checkpoint
    checkpoint_path = "/home/project/ldr_image_to_ldr_shading/LDR_image_to_LDR_shading_hyperism/train_vae_mithlesh/ldr_to_sh_flow_1/best_flow_model_ckpt.pth"
    ...
    # add more transforms if needed, e.g., ToTensor(), Normalize()
    ])
    triplet_dataset = HDRAlbedoShadingTripletDataset(
        hdr_folder=args.hdr_folder,
        albedo_folder=args.albedo_folder,
        shading_folder=args.shading_folder,transform=transform
    )
    ...
    #methods = [ "bosh3", "fehlberg2", "adaptive_heun", "euler", "midpoint", "heun2", "heun3", "rk4", "explicit_adams", "implicit_adams", "fixed_adams", "scipy_solver"]
    methods=['euler']
    ...
    if len(results_df) > 0:
        ...
        metrics_to_show = ['Albedo_SI_MSE', 'Albedo_SI_RMSE', 'Albedo_LMSE', 'Albedo_SSIM', 'Albedo_RMSE','Albedo_PSNR',
                          'Shading_SI_MSE', 'Shading_SI_RMSE', 'Shading_LMSE', 'Shading_SSIM', 'Shading_RMSE','Shading_PSNR']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hdr_folder', type=str, default="/mnt/zone/B/mithlesh/dataset/arap/processed_arap_png/ldr_images",
                       help='Path to folder containing HDR images')
    parser.add_argument('--albedo_folder', type=str, default="/mnt/zone/B/mithlesh/dataset/arap/processed_arap_png/albedo_images",
                       help='Path to folder containing albedo images')
    parser.add_argument('--shading_folder', type=str, default="/mnt/zone/B/mithlesh/dataset/arap/processed_arap_png/shading_images",
